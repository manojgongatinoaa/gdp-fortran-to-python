...
def enter_exp_no() -> str:
    ...
    print('\n')
    global g_exp_no
    g_exp_no = input(f"{' ' * 9}{'Enter experiment number to search for: '}")
...
def active_buoys():
    ...
    file_for05X = dprint_active_by_experiment(g_exp_no, 
                                g_directory_file, 
                                g_jd_to_date)
...
def dead_buoys():
    ...
    global g_directory_file
    # Formats and writes records
    file_for05X = dprint_dead_by_experiment(g_exp_no, 
                            g_directory_file, 
                            g_jd_to_date)
    write_file(file_for05X)
...
def all_buoys():
    ...
    global g_directory_file
    # Formats and writes records
    file_for05X = dprint_all_by_experiment(g_exp_no, 
                            g_directory_file, 
                            g_jd_to_date)
    write_file(file_for05X)
...
# It creates files listing buoys with a specified experiment number.
...
def list_by_experiment_menu(jd_to_date, count, created_files) -> int:
    enter_exp_no()
    ...
    text = "M E N U (by experiment number)"
    title = '\n' + f"{' ' * 9}{text}"  + '\n'
