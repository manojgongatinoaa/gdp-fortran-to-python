...
# Identifies buoys that have stopped reporting data, are more than 30 days behind 
# the latest database update but have not yet been officially flagged as "dead" 
# in the DIR-File file.
def subchkdead(directory_file):
    # File to write logging summaries.
    file_name = "chkdead.out"
    results = []
    record = ''
    ...
    while True:
        ...
        if (common.is_valid_date(user_input, "%m/%d/%Y")):
            ...
            if len(spl) > 2:
                ...
                month = spl[0]
                day = spl[1]
                header = ' the following buoys are behind and not declared dead in dirfl' + '\n'
                ...
                for row in directory_file:
                    ...
                    if (death_code == 0 and end_time > 0.0 and (end_time < j_last_date - 30.0)):
                        # If a buoy is marked alive and end time is more than 30 days older 
                        # than the latest update date, it is flagged as missing.
                ...
                if len(results) > 0:
                    ...
                    text = f"{' file: '}{file_name}{' is empty, all the buoys are correctly flaged as dead.'}"
                print('\n')
                print(text)
