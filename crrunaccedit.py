...
# This function automates the creation of a shell script (mainedit.sh).
# This script acts as a "runstream" to feed inputs into an external program called mainedit.exe.
def crrunaccedit():
    print('\n')
    ...
    try:
        ...
        if (len(abs_numbers) > 0):
            runstream_file = destination_path + "mainedit.sh"
            ...
            header.append("/phodnet/drifter/programs/mainedit.exe << end > mainedit.out") #  execution command for mainedit.exe
            # Insert header at the beginning.
            abs_numbers[:0] = header

            food = []
            food.append(" 9999") # serves as a termination signal or exit code for the mainedit.exe program.
            food.append("end") # to close the Linux Here Document (<< end), completing the script's input stream.
            # Add food to the end.
            abs_numbers.extend(food)

            # Creates or overwrites the output shell script mainedit.sh
            ...
            print('\n')
            print(f"{'Output from running mainedit.sh is: '}")
            print (f"{' ' * 4}{destination_path}{'mainedit.out'}")
